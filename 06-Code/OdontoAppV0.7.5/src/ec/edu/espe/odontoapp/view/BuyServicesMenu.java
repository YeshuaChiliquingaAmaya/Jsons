                ...
                for(int i = 0; i < consultory.getPatients().size(); i++){
                    if(idOption == consultory.getPatients().get(i).getId()){
                        consultory.getPatients().get(i).setDebtToPay(finalResult);
                         ...
                         if (result > 200){
                           descuento = result * 0.15;
                           totalDescuent = result - descuento;
                            String Descuent = String.valueOf(totalDescuent);
                            finalResult = Descuent;
                            System.out.println("For the patient " + consultory.getPatients().get(i).getName() + " has been added a debt with descuent of " + Descuent);
                         }else{
